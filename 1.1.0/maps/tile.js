class Tile{
    ...
    drawVworld(centerLatLng, config, vworldProfile, callback){
        ...
        var ctx = canvas.getContext("2d");
        var nw = this.getNW(config, centerLatLng);
        var startLatLng = new mapshot.coors.LatLng(
        ...
        var se = new mapshot.coors.LatLng(
            nw.getX() + this.getWidthBetweenBlock(),
            nw.getY() - this.getHeightBetweenBlockWithLogo()  
        );

        // vworldProfile.setBbox(nw.getX() + "," + se.getY() + "," + se.getX() + "," + nw.getY());
        ...
        for (var i = 0; i < sideBlockCount; i++) {
            for (var j = 0; j < sideBlockCount; j++) {
                ...
                if (i + 1 === sideBlockCount && j === 0) {
                    ...
                    se.init(
                        startLatLng.getX() + this.getWidthBetweenBlock() / 2,
                        startLatLng.getY() - this.getHeightBetweenBlockNoLogo() / 2
                    );
                ...
                (function (_order, _image) {
                    ...
                    _image.onload = function () {
                        ctx.drawImage(_image, 0, 0, defaultBlockWidth, defaultBlockHeight - logoRemover, xPos, yPos, canvasBlockSize, canvasBlockSize);
                        complete++;
                        document.body.dispatchEvent(tileImageOnLoadEvent);
